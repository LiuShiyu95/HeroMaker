...
class SMPLX(SMPLH):
    ...
    def forward(
        ...
        **kwargs
        ...
        if delta is None:
            delta = torch.zeros([expression.shape[0], 10475, 3], dtype=torch.float32, device="cuda:0")
        ...
        full_pose += self.pose_mean
        if pose_type == "t-pose-front":
            global_orient *= 0
            pi_tensor = torch.tensor(np.pi, dtype=torch.float32).cuda()
            global_orient[:,0] = global_orient[:,0] + pi_tensor
            full_pose = torch.cat(
                [
                    global_orient,
                    body_pose * 0,
        ...
        elif pose_type == "t-pose-back":
            global_orient *= 0
            pi_tensor = torch.tensor(np.pi, dtype=torch.float32).cuda()
            global_orient[:,2] = global_orient[:,2] + pi_tensor
            # global_orient[:,0] = global_orient[:,0] + pi_tensor
            full_pose = torch.cat(
                [
                    global_orient,
            ...
            body_pose[:, 17] = torch.tensor([ -90 * np.pi / 180., 0., 0.])
            body_pose[:, 18] = torch.tensor([ -90 * np.pi / 180., 0., 0.])
        ...
        vertices, joints = lbs(shape_components, full_pose, self.v_template,
                               ...
                               self.J_regressor, self.parents,
                               self.lbs_weights, delta=delta, pose2rot=pose2rot,
                               )
    ...
    def forward(
        ...
        **kwargs
        ...
        if transl is None:
            transl = torch.zeros([batch_size, 3], dtype=dtype, device=device)
        if delta is None:
            delta = torch.zeros([batch_size, 6890, 3], dtype=dtype, device=device)
        ...
        vertices, joints = lbs(shape_components, full_pose, self.v_template,
                               shapedirs, self.posedirs,
                               self.J_regressor, self.parents,
                               self.lbs_weights, delta, pose2rot=True)
