...
class SMPLRenderer(nn.Module):
    def __init__(self, face_path='/public/home/liushy/im/assets/pretrains/smpl_faces.npy',
                 uv_map_path='/public/home/liushy/im/assets/pretrains/mapper.txt', map_name='uv_seg', tex_size=3, image_size=256,
    ...
    def render_fim_wim(self, info):
        ...
        cha = info['cha']
        scale = info['scale']
        vertices = info['yuan_mesh']
        faces = info['faces']

        vertices = vertices + cam[:, None, :] + cha
        # proj_verts = self.proj_func(vertices, cam)
        # print("scale===========",scale)
        # print("gamma==========",gamma)
        proj_verts = vertices * scale / gamma / 3

        # print(proj_verts.shape,"======proj_verts.shape")
        # flipping the y-axis here to make it align with the image coordinate system!
        proj_verts[:, :, 1] *= -1
        vertices = nr.look_at(proj_verts, self.eye)
    ...
    def render_fim_wim_new(self, vertices, faces, gamma, cha, scale):
        vertices = vertices 
        proj_verts = vertices * scale / gamma / 2.9
        proj_verts = proj_verts + cha
        proj_verts[:, :, 1] *= -1
        vertices = nr.look_at(proj_verts, self.eye)
    ...
    def render_fim_wim_new_1(self, vertices, faces, gamma, cha, scale):
        # print(scale / gamma / 2.9)
        vertices = vertices 
        proj_verts = vertices * scale / gamma / 3
        proj_verts = proj_verts + cha 
        proj_verts[:, :, 1] *= -1
    ...
    def render_fim_wim_new_1(self, cam, vertices, faces=None):
        if faces is None:
            bs = cam.shape[0]
            faces = self.faces.repeat(bs, 1, 1)

        # set offset_z for persp proj
        # 这个地方干啥了，这个狄梵是先把cam的参数拆分成了scale和trans这两个参数，就是先加上对应的偏移量，同时乘上了scale，再调整了距离相机的位置。这个是投影到2d的操作？
        proj_verts = self.proj_func(vertices, cam)
        # flipping the y-axis here to make it align with the image coordinate system!
        # 翻转y轴，把坐标系调整到图片的坐标系
        proj_verts[:, :, 1] *= -1
        print(proj_verts.shape)
        # calculate the look_at vertices.
        # 这个是相机坐标系下的mesh，大概十对应到我处理完之后的情况，那他这个eye是啥咧
        # self.eye = [0, 0, -(1. / np.tan(np.radians(self.viewing_angle)) + 1)] 这个是eye的值，他的xy是0，然后z是一个有关观察角度的参数，这个后续再看看这是什么个意思。
        # 嗯。。大概是再eye这个视角去管查，生成一个新的mesh，但是他为什么要这么做呢，后续再看看
        vertices = nr.look_at(proj_verts, self.eye)
        print(vertices.shape,"======vertices.shape")
        # rasterization
        # 这个搞完之后大概就是投影到相机坐标系下的mesh和faces去找对应关系
        faces = nr.vertices_to_faces(vertices, faces)
        fim, wim = nr.rasterize_face_index_map_and_weight_map(faces, self.image_size, False)
        return faces, fim, wim
