...
def render_mesh_white(img, mesh, face, cam_param):
    # mesh
    mesh = trimesh.Trimesh(mesh, face)
    rot = trimesh.transformations.rotation_matrix(
	np.radians(180), [1, 0, 0])
    mesh.apply_transform(rot)
    material = pyrender.MetallicRoughnessMaterial(metallicFactor=0.0, alphaMode='OPAQUE', baseColorFactor=(0.812, 0.812, 1.0, 0.93))
    mesh = pyrender.Mesh.from_trimesh(mesh, material=material, smooth=False)
    scene = pyrender.Scene(ambient_light=(0.3, 0.3, 0.3))
    scene.add(mesh, 'mesh')
    ...
    image_white_bg = np.ones_like(img) * 150
    image_white_bg = rgb * valid_mask + image_white_bg * (1 - valid_mask)
    image_white_bg = img
    return image_white_bg


def render_mesh_black(img, mesh, face, cam_param):
    # mesh
    mesh = trimesh.Trimesh(mesh, face)
    rot = trimesh.transformations.rotation_matrix(
	np.radians(180), [1, 0, 0])
    ...
    rgb, depth = renderer.render(scene, flags=pyrender.RenderFlags.RGBA)
    rgb = rgb[:,:,:3].astype(np.float32)
    valid_mask = (depth > 0)[None,:,:].astype(np.float32)
    # sys.exit()
    # # save to image
    # img = rgb * valid_mask + img * (1-valid_mask)
    
    # image_black_bg = np.zeros_like(img) * 255
    # image_white_fg = np.ones_like(img) * 255
    # image_black_bg = rgb * valid_mask + image_black_bg * (1 - valid_mask)
    # image_depth = image_white_fg * valid_mask + image_black_bg * (1 - valid_mask)

    return valid_mask


def render_mesh_deg(img, mesh, face, cam_param, deg=0):
    # mesh
    cur_mesh = mesh.copy()
    mesh = trimesh.Trimesh(mesh, face)
    ...
    material.baseColorFactor = [0.25, 0.4, 0.65, 1.0]
    ...
    img_white_bg = np.ones_like(img)*255
    img_white_bg = rgb * valid_mask + img_white_bg * (1 - valid_mask)

    return img_white_bg


def render_depth_black(img, mesh, face, cam_param):
    # mesh
    mesh = trimesh.Trimesh(mesh, face)
    rot = trimesh.transformations.rotation_matrix(
	np.radians(180), [1, 0, 0])
    ...
    image_black_bg = np.zeros_like(img) * 255
    image_white_fg = np.ones_like(img) * 255
    image_black_bg = image_white_fg * valid_mask + image_black_bg * (1 - valid_mask)

    return image_black_bg